...
if (/^2كيريتو$/i.test(m.text) ) { //sem prefixo 
 conn.reply(m.chat, `القلب ❤️`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
}
 ...
 conn.reply(m.chat, ` قول نعم يا حب 🫶🏻 🐦❤`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
   ...
   if (/^غون|غوني|غوون$/i.test(m.text) ) { //sem prefixo
 ...
 conn.reply(m.chat, `اوهايو كوزيماس `, m) //wm, null, [['Menu', '#menu']], m) botones :V
   ...
   if (/^كيريتو|غينتوكي$/i.test(m.text) ) { //sem prefixo 
 conn.reply(m.chat,'صديق غون الروح بروح 😂❤', m) //wm, null, [['Menu', '#menu']], m) botones :V
